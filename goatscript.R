...
###Can representative clustering, virulence-factor screening, and conserved motif identification uncover putative functions in hypothetical proteins of Acinetobacter baumannii?
...
virulence_hits <- read_tsv('/Users/nicolasbailly/Documents/side_projects/virulence_results.tsv') 
cluster_protein <-readLines('/Users/nicolasbailly/Documents/side_projects/clustered_70.faa.clstr') 
hypothetical_proteins <- readAAStringSet('/Users/nicolasbailly/Documents/side_projects/hypothetical_proteins_filtered.faa') 
...
##Make an organized table for cluster_protein
?str_detect
...
merged_virulence_cluster <- merged_virulence_cluster %>% filter (Perc_Identity > 90)
#Filter the merged data set to only keep proteins  which virulence hits had a percent identity > 90

virulence_summary <- merged_virulence_cluster %>%
  group_by(Protein_ID) %>%
  summarise(
    Num_VF_Hits = n (),
...
motif_results <- read_fwf(
  "/Users/nicolasbailly/Documents/side_projects/hmmscan_results.tbl",
  fwf_empty("/Users/nicolasbailly/Documents/side_projects/hmmscan_results.tbl"), skip = 3
)
print(motif_results)
...
motif_results <- motif_results %>% 
  ...
  arrange(E_value)
print(motif_results)
#Filter and organizethe imported data set to retain the observations with lower e-values. Threshold chosen arbitrarily. 
...
#Graphical representation of the type of motif associated with each sequence. The e-values were plotted as a function of -log(10) to standardize the representation.
